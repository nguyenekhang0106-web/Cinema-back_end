...
import { Button, Card, Col, Modal, Row, Space, Tag, Typography, Spin, message } from "antd";
...
export function MovieGrid({
  ...
  const [selectedMovieForBooking, setSelectedMovieForBooking] = useState<any | null>(null);
  ...
  const getGroupedShowtimes = () => {
    ...
    const activeShowtimes = showtimes.filter(st => {
      ...
      return st.movieId === selectedMovieForBooking.id &&
            ...
            <Card
              ...
              cover={
                <div className="relative aspect-[3/4] overflow-hidden group">
                  ...
                  <Link href={localizeHref(`/phim/${movie.slug || movie.id}`, locale)} className="absolute inset-0 z-10 block">
                    {movie.posterUrl && (
                      <Image
                        src={movie.posterUrl}
                        alt={movie.title}
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/15 to-transparent p-4 z-20 transition-opacity duration-300 group-hover:opacity-0 pointer-events-none">
                    <div className="flex items-start justify-between gap-3">
                      {movie.bookingLabel && <Tag color="red">{movie.bookingLabel}</Tag>}
            ...
            >
              ...
              <Space direction="vertical" size={10} className="w-full">
                <Typography.Text style={{ color: "#7b6a58", fontSize: '13px' }}>
                  {movie.release || dayjs(movie.releaseDate).format("DD-MM-YYYY")} | {movie.duration || `${movie.durationMin} phút`}
                </Typography.Text>
                ...
                <Space direction="vertical" size={8} className="w-full mt-1">
                  <Link href={localizeHref(`/phim/${movie.slug || movie.id}`, locale)}>
                    <Button block>{dictionary.home.detail}</Button>
                  </Link>
                  {showBooking && (
                    <Button block type="primary" onClick={() => handleOpenBookingModal(movie)}>
                      {dictionary.home.book}
                    </Button>
                  )}
                </Space>
      ...
      >
        {selectedMovieForBooking && (
          <div className="flex flex-col md:flex-row gap-6 mb-6">
            <img src={selectedMovieForBooking.posterUrl} alt="Poster" className="w-32 rounded-lg shadow-md hidden md:block object-cover aspect-[2/3]" />
            <div>
              <Typography.Title level={3} style={{ color: '#a61d24', margin: 0, textTransform: 'uppercase' }}>
                {selectedMovieForBooking.title}
              </Typography.Title>
              <p className="text-gray-500 mt-2 text-sm">
                <ClockCircleOutlined className="mr-1"/> {selectedMovieForBooking.duration || `${selectedMovieForBooking.durationMin} phút`} | Thể loại: {selectedMovieForBooking.genre}
              </p>
            </div>
          </div>
        )}
